...
class UberSurvivalCoverageModel(RelocationModel):
...
class MaxExpectedSurvivalCoverageRelocator(RelocationModel):
    ...
    def relocate(self,
        ...
        z = [model.addVar(vtype=grb.GRB.BINARY, name='z_' + node) for node in D]
        ...
        # [j, i] matrix respresenting S_{i,j} * d_i
        coefficients = D_rates[1].loc[t+1, D].values * (filtered_survival + z)
        ...
        Const_3 = {i: model.addConstr(lhs=grb.quicksum(x[C.index(u)][i] for u in C),
                                      sense=grb.GRB.LESS_EQUAL,
                                      rhs=1,
                                      name='Const_2_{}'.format(i)) for i, d_node in enumerate(D)}
